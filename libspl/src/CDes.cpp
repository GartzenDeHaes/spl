...
#include <spl/types.h>             /* for byte and u32 typedefs */
...
#define G10ERR_WEAK_KEY -3

using namespace spl;

static void desSetPassword(des_ctx context, const String& password);
...
static void des_key_schedule (const byte *, uint32 *);
...
#define WRITE_64BIT_DATA(data, left, right)					\
    data[0] = (byte)((left >> 24) &0xff); data[1] = (byte)((left >> 16) &0xff); 		\
    data[2] = (byte)((left >> 8) &0xff); data[3] = (byte)(left &0xff);				\
    data[4] = (byte)((right >> 24) &0xff); data[5] = (byte)((right >> 16) &0xff);		\
    data[6] = (byte)((right >> 8) &0xff); data[7] = (byte)(right &0xff);
 ...
 */
static void
des_key_schedule (const byte * rawkey, uint32 * subkey)
{
 ...
 * to 'mode'.
 */
static int des_ecb_crypt (struct _des_ctx *ctx, const byte * from, byte * to, int mode)
{
  uint32 left, right, work;
  uint32 *keys;
  ...
  while(left <= right)
    {
      ...
      if ( !(cmp_result=working_memcmp((char *)work, (char *)weak_keys[middle], 8)) )
	  return -1;
...
static void _encryptBuffer(des_ctx context, Array<byte>& data, const bool encrypt)
{
	byte chunk[8];
	Array<byte> to(8);
	int x;
	...
	for ( x = 0; x < len; x+=8) 
	{
		...
		if (encrypt) 
		{
			des_ecb_encrypt(context, chunk, to.Data());
		}
		else 
		{
			des_ecb_decrypt(context, chunk, to.Data());
		}
		Array<byte>::CopyBinary(to, 0, data, x, 8);
	}
	if ( (len % 8) != 0 ) 
	{
		byte out[8];
		...
		for (x = 7; x > 0; x--) 
		{
			chunk[x-1] = data[len-x];
		}
		if (encrypt) 
		{
			des_ecb_encrypt(context, chunk, out);
		}
		else 
		{
			des_ecb_decrypt(context, chunk, out);
		}
		for (x = 7; x > 0; x--) 
		{
			data[len-x] = out[x-1];
...
static void desSetPassword(des_ctx context, const String& password)
{
	...
	des_setkey(context, pass);	
}

static bool desEncryptBinaryCtx( des_ctx context, const Array<byte>& data, Array<byte>& output)
{
	int outlen;
	...
	{
		outlen = data.Length() + 8 - data.Length() % 8;
	}
	
	if (output.Length() != outlen)
	{
		output = Array<byte>(outlen);
	}
	else
	{
		output.Clear();
	...
	_encryptBuffer(context, output, true);
	return true;
}

bool desDecryptBinaryCtx( des_ctx context, Array<byte>& data )
{
	/* decrypt */
	_encryptBuffer(context, data, false);
	ASSERT( sizeof(byte) == sizeof(char) );
	return true;
}

bool desEncryptBinary(const String& password, const Array<byte>& data, Array<byte>& output)
{
	des_ctx context;
	desSetPassword( context, password );
	return desEncryptBinaryCtx(context, data, output);
}

bool desDecryptBinary( const String& password, Array<byte>& data )
{
	des_ctx context;
	desSetPassword( context, password );
	return desDecryptBinaryCtx(context, data);
}

static RefCountPtr<String> desEncryptStringCtx(des_ctx ctx, const Array<byte>& data)
{
	int datalen8 = data.Length() + 8 - data.Length() % 8;
	Array<byte> data8(datalen8);

	Array<byte>::CopyBinary(data, 0, data8, 0, data.Length());

	_encryptBuffer(ctx, data8, true);
	return String::Base64Encode((const char *)data8.Data(), datalen8);
}

static RefCountPtr<Array<byte> > desDecryptStringCtx(des_ctx context, const String& data)
{
	RefCountPtr<Array<byte> > bins = String::Base64Decode(data);

	/* decrypt */
	_encryptBuffer(context, bins, false);
	
	return bins;
}

DES::DES(const String& password)
{
	desSetPassword( m_ctx, password );
}

void DES::EncryptBinary(const Array<byte>& data, Array<byte>& output)
{
	if ( ! desEncryptBinaryCtx(m_ctx, data, output) )
	{
		throw OutOfMemoryException();
	}
}

void DES::DecryptBinary(Array<byte>& data )
{
	if ( ! desDecryptBinaryCtx(m_ctx, data) )
	{
		throw OutOfMemoryException();
	}
}

RefCountPtr<Array<byte> > DES::DecryptString(const String& data)
{
	RefCountPtr<Array<byte> > output = desDecryptStringCtx(m_ctx, data);
	return output;
}

RefCountPtr<String> DES::EncryptString(const Array<byte>& data)
{
	return desEncryptStringCtx(m_ctx, data);
}

RefCountPtr<Array<byte> > DES::DecryptString(const String& password, const String& data)
{
	des_ctx context;
	desSetPassword( context, password );

	return desDecryptStringCtx(context, data);
}

RefCountPtr<String> DES::EncryptString(const String& password, const Array<byte>& data)
{
	des_ctx context;

	desSetPassword( context, password );
	
	return desEncryptStringCtx( context, data );
}

void DES::EncryptBinary(const String& password, const Array<byte>& data, Array<byte>& output)
{
	if ( !desEncryptBinary(password, data, output) )
	{
		throw OutOfMemoryException();
	}
}

void DES::DecryptBinary(const String& password, Array<byte>& data)
{
	if ( !desDecryptBinary(password, data) )
	{
		throw OutOfMemoryException();
	}
}
