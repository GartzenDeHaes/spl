...
{
	register char *tem = (char *)malloc	(size);
...
{
	register char *tem = (char *)realloc (ptr, size);
...
static char	*tparam1 (	const char *str,
	char *outstr,
	int	len, 
	char *up, 
	char *left, 
...
/* VARARGS 2 */
char *tparam (const char *str, char *outstring, int len, ...)
{
	va_list arg_list;
	int	arg[4];
	...
	arg[1] = va_arg (arg_list, int);
	arg[2] = va_arg (arg_list, int);
	arg[3] = va_arg (arg_list, int);
	return tparam1 (str, outstring,	len, NULL, NULL, arg);
}

char *BC = NULL;
char *UP = NULL;

static char	tgoto_buf[150];
...
char *tgoto (const char *cm, int hpos, int vpos)
{
	...
	args[0]	= vpos;
	args[1]	= hpos;
	return tparam1 (cm,	tgoto_buf, 150, UP, BC, args);
}
...
)
{
	...
	while (1)
	{
		...
		{
			register char *newc;
			int	offset = (int)(op -	outstr);

			if (outlen == 0)
			{
				outlen = len + 40;
				newc	= (char	*) xmalloc (outlen);
				bcopy (outstr, newc, offset);
			}
			else
			{
				outlen *= 2;
				newc	= (char	*) xrealloc	(outstr, outlen);
			}
