...
SqliteCommand::SqliteCommand(const SqlLiteCommand& cmd)
: Command(cmd), m_db(cmd.m_db), m_stmt(NULL)
{
}

SqliteCommand::SqliteCommand(void *db, const String& cmdtxt)
...
SqliteCommand& SqliteCommand::operator =(const SqlLiteCommand& cmd)
{
...
void SqliteCommand::BindParameters()
{
	Prepare();
...
RecordSetPtr SqliteCommand::ExecuteQuery()
{
	BindParameters();
